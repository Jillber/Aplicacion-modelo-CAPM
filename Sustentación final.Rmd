---
title: "Aplicación Econometría financiera"
...
```
```{r}
rendimiento1<-c(mean(SP), mean(BABA), mean(ECL), mean(CAT),mean(TWTR))
volatilidad1<-c(sd(SP), sd(BABA), sd(ECL), sd(CAT),sd(TWTR))
...
colnames(tabla2)<-c("SP","BABA","ECOLAP","CATTERPILLAR","TWITTER")
tabla2*100

```
```{r}
sharpe1<-(rendimiento1-mean(Rf))/volatilidad1
sharpe1
```
```{r}
